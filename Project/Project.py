...
class character():
    ...
    def __init__(self):
        ...
        pygame.image.load('character/run2.png'),pygame.image.load('character/run3.png'),
        pygame.image.load('character/run4.png'),pygame.image.load('character/run5.png'),
        pygame.image.load('character/run6.png')]
        self.walkLeft = [pygame.transform.flip(x, True, False) for x in self.walkRight]
    ...
    def draw(self,win):
        ...
        if self.x >= game.screenWidth//2 and game.onScreenX<game.stageWidth - game.startScrollingPosX:
           self.x = game.screenWidth//2

        elif game.onScreenX>=game.stageWidth - game.startScrollingPosX:
            self.x = game.screenWidth//2 + game.onScreenX - (game.stageWidth - game.startScrollingPosX)
...
class button():
    ...
    def draw(self,win):
        pygame.draw.rect(win,self.color,(self.x,self.y,400,60))

    #checks whether the button is clickable by the mouse
    def isClickable(self,pos):
        if pos[0] >= self.x and pos[0] <= self.x + 300 and pos[1] >= self.y and pos[1] <= self.y + 60:
            return True
        return False
...
def Menu():
    ...
    SinglePlayer = button(200,200,(255,0,0))
    Multiplayer = button(200,400,(255,0,0))
    Quit = button(200,600,(255,0,0))


    menupic = pygame.image.load("menu.jpg")
    win.blit(menupic,(0,0))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if SinglePlayer.isClickable(position):
                    run = False
                    return True
                if Multiplayer.isClickable(position):
                    messagebox.showinfo("Coming Soon in Checkpoint 2")
...
def drawPlatforms():
    ...
    for platoi in game.platforms:
        ...
        if player1.x + 90 >= platoi.x and player1.x <= platoi.x + 275:
            if player1.y + 120 <= platoi.y and platoi.y  - player1.y - 140 < 50:
        ...
        if not game.onPlatform:
            if player1.goingRight:
                ...
                if player1.x + 160 >= platoi.x and player1.x <= platoi.x + 230 and not player1.isJumping:
                    if player1.y + 140 > platoi.y + 20:
                        
                        player1.rightvelocity = 0
                        player1.x = platoi.x - 160
                        game.onScreenX -= 45
            ...
            if player1.goingLeft:
                if player1.x <= platoi.x + 290 and player1.x > platoi.x:
                    if player1.y + 140 > platoi.y + 20 and not player1.isJumping:
...
def drawEnemies():
    ...
    for enemy in game.enemies:
        ...
        if enemy.goingLeft:
            if enemy.x <= player1.x + 110 and enemy.x >= player1.x:
            ...
            if enemy.x + 90 <= player1.x + 110 and enemy.x + 100 >= player1.x:
                if player1.y + 140 > enemy.y and player1.y < enemy.y + 90:
...
class currentLevel():
    def __init__(self):
        ...
        pygame.display.set_caption("15-112 Project")
